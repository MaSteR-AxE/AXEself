...
local Clang = redis:get(Chash)
	if (matches[1]:lower() == 'calc' and not Clang) or (matches[1]:lower() == 'ماشین حساب' and not Clang) and matches[2] then 
		if msg.to.type == "all" then 
			return 
       end
		return edit_msg(msg.to.id, msg.id, calc(matches[2]), "html")
	end
--------------------------------
	if (matches[1]:lower() == 'praytime' and not Clang) or (matches[1]:lower() == 'ساعات شرعی' and not Clang) then
		if matches[2] then
			city = matches[2]
...
--------------------------------
	if (matches[1]:lower() == 'tophoto' and not Clang) or (matches[1]:lower() == 'تبدیل به عکس' and not Clang) and msg.reply_id then
			...
			function tophoto_cb(arg,data)
				if data.content_.sticker_ then
					...
					if file_exi(tostring(name), tostring(apath), tostring(pasvand)) then
						...
						tdcli.sendPhoto(msg.to.id, 0, 0, 1, nil, pfile, "🌹AxE🦋", dl_cb, nil)
					else
...
--------------------------------
	if (matches[1]:lower() == 'tosticker' and not Clang) or (matches[1]:lower() == 'تبدیل به استیکر' and not Clang) and msg.reply_id then
			...
			function tosticker_cb(arg,data)
				if data.content_.ID == 'MessagePhoto' then
					...
					if file_exi(file..'_(1).jpg', tcpath..'/data/photo', 'jpg') then
						...
						tdcli.sendDocument(msg.chat_id_, 0, 0, 1, nil, pfile, "🌹AxE🦋", dl_cb, nil)
					else
...
--------------------------------
	if (matches[1]:lower() == 'weather' and not Clang) or (matches[1]:lower() == 'اب و هوا' and not Clang) then
		city = matches[2]
		local wtext = get_weather(city)
		if not wtext then
			wtext = 'مکان وارد شده صحیح نیست'
		end
		return edit_msg(msg.to.id, msg.id, wtext, "html")
	end
--------------------------------
	if (matches[1]:lower() == 'time' and not Clang) or (matches[1]:lower() == 'ساعت' and not Clang) then
		...
		tdcli.sendDocument(msg.to.id, 0, 0, 1, nil, file, 'time', dl_cb, nil)

	end
--------------------------------
	if (matches[1]:lower() == 'voice'and is_sudo(msg)) or (matches[1]:lower() == 'تبدیل به صدا') then
 local text = matches[2]
  ...
  local file = download_to_file(url,'AxE.mp3')
 				tdcli.sendDocument(msg.to.id, 0, 0, 1, nil, file, '🌹voice : '..text.."🦋", dl_cb, nil)
   end
end

 --------------------------------
	if (matches[1]:lower() == 'tr' and not Clang) or (matches[1]:lower() == 'ترجمه' and Clang) then 
		url = https.request('https://translate.yandex.net/api/v1.5/tr.json/translate?key=trnsl.1.1.20160119T111342Z.fd6bf13b3590838f.6ce9d8cca4672f0ed24f649c1b502789c9f4687a&fo	rmat=plain&lang='..URL.escape(matches[2])..'&text='..URL.escape(matches[3]))
		data = json:decode(url)
		return edit_msg(msg.to.id, msg.id, 'زبان : '..data.lang..'\nترجمه : '..data.text[1]..'\n 🌹AxE🦋', "html")
	end
--------------------------------
	if (matches[1]:lower() == 'short' and not Clang) or (matches[1]:lower() == 'لینک کوتاه' and not Clang) then
...
--------------------------------
	if (matches[1]:lower() == 'استیکر'and is_mod(msg) and not Clang) or (matches[1]:lower() == 'sticker' and is_mod(msg) and not Clang) then
...
--------------------------------
	if (matches[1]:lower() == 'عکس'and is_sudo(msg) and not Clang) or (matches[1]:lower() == 'photo'and is_sudo(msg) and not Clang) then
		local eq = URL.escape(matches[2])
		local w = "500"
		local h = "500"
		local txtsize = "100"
...
if matches[1] == "راهنمای سرگرمی" and not Clang then
local hash = "gp_lang:"..msg.to.id
local lang = redis:get(hash)
if not lang then
helpfun_fa = [[
...
return edit_msg(msg.to.id, msg.id, helpfun_fa.."\n🌹AxE🦋", "html")
end
